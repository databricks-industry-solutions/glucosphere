...
export default function MetricsExplained() {
  ...
  const [baselineSource, setBaselineSource] = useState('real_from_source');
  const [baselineSourceDetail, setBaselineSourceDetail] = useState('');
  useEffect(() => {
    getConfig().then(cfg => {
      setCatalog(cfg.catalog || '');
      setSchema(cfg.schema || '');
      setBaselineSource(cfg.baseline_source || 'real_from_source');
            ...
            <p className="text-sm text-slate-400 leading-relaxed mt-3">
              ...
              {baselineSource === 'synthetic' ? (
                <>
                  ...
                  + monitoring stack against a clean baseline. A <span className="font-mono text-cyan-400">real_from_source</span>{' '}
              ...
              ) : baselineSource === 'real_from_table' ? (
                <>
                  This deployment is running in <span className="font-mono text-cyan-400">baseline_source=real_from_table</span>{' '}
                  ...
                  <span className="font-mono text-cyan-400">real_from_source</span> modes are also available —
                  see <span className="font-mono text-cyan-400">baseline_source</span> in the deploy docs.
                </>
              ) : (
                <>
                  This deployment is running in the default <span className="font-mono text-cyan-400">baseline_source=real_from_source</span>{' '}
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400 font-mono">
                  ...
                  {' '}job — see the job page for the run history. PNG is read live from UC Volume
