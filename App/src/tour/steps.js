...
export const TOUR_STEPS_INTERACTIVE = [
  ...
  { route: '/diabetes-coach', selector: '[data-tour="coach-risk"]', title: '③ Assess — per patient', interactive: true, body: 'Down to one patient: their near-term (15/30-min) glucose forecast from the XGBoost model. ▶ Try it: search PSEUDO_0000168 — in range now, but the model forecasts a climb into hyperglycemia over the next 30 min.' },
  { route: '/diabetes-coach', selector: '[data-tour="assistant-fab"]', title: 'Open the assistant', interactive: true, body: "A built-in AI assistant on every page. On the Coach view it opens on the CGM data (Genie) tab — the next two steps show Genie first, then the Fast ⇄ MAS engine switch on the Device-support tab." },
