...
export const TOUR_STEPS_INTERACTIVE = [
  ...
  { route: '/firmware-lifecycle', selector: '[data-tour="firmware-act"]', title: '→ ACT', body: 'Name the culprit rollouts (4.0 + 4.0.3), size each recall/outreach list, and flag them for rollback — the handoff from diagnosis to action.' },
  { route: '/triage', selector: '[data-tour="triage-queue"]', requiresLakebase: true, interactive: true, title: '→ ACT — work the queue', body: 'The live alert queue (Lakebase-backed Postgres — the app\'s only write path). ▶ Try it: Ack an alert, then 🛢 Verify in Postgres → "Peek right here" — your click appears as a database row under the queue. Expand a row for patient context + audit trail, or filter to FW 4.0 and bulk-resolve the cohort as "🔧 Firmware rolled back". ⟲ Reset demo restores 600 open alerts (and truncates the Postgres tables — verifiably).' },
