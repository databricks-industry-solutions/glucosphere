...
- **`mas_endpoint` / `ka_endpoint` / `genie_space_id`** bundle variables: the App-assistant agent coords can now be set in `.env.bundle.<target>` (`BUNDLE_VAR_*`) so `render_app_yaml.py` auto-fills `app.yaml` from the env file alone — no `--mas-endpoint` / `--ka-endpoint` / `--genie-space-id` flags needed (flag still wins; mirrors the `existing_warehouse_id` precedence). For targets that render from their env file (e.g. a booth). Documented in `.env.bundle.example` + DEPLOY.md.
...
### Added — "Interactive" guided-tour variant (pause → try it → resume) + tour polish
- A **third tour option**. The chooser ships **Quick + Interactive** (Full walkthrough commented out as backup — Interactive is Full + pause-to-try + the metrics/About close, so it subsumes Full). **Every step** in the Interactive variant offers **"Try it yourself — I'll wait here"** (its defining feature): the overlay steps aside so the page is fully clickable, a prominent floating **Resume · Step N/M** button (opaque; sits just left of the assistant "Ask" FAB, and shifts clear of the panel when it's open — tracking the panel's real state) appears, and clicking it returns to the *same* step — no restart from step 1. So you can actually exercise whatever's spotlighted: the firmware heatmap's **In-incident ⇄ Fleet-wide scope toggle**, the calibration-drift **cell drill-down**, the Population-Risk **normalize toggle + roster filter**, the Device-Support drill-in **AI "Deeper Analysis"**, the per-patient **search**, the assistant's **⚡ Fast ⇄ 🤖 MAS engine switch** + **Genie data tab**, and the About **"under the hood" workspace deep-links**. Previously the dim overlay swallowed clicks and closed the tour, so these never got showcased. The arc adds explicit stops on the engine switch + Genie tab and closes on **Metrics Explained → About** before returning home (new `data-tour` anchors: `assistant-engine`, `assistant-genie-tab`, `metrics-explained`, `about-hood`). The assistant steps **auto-open the panel** (a `glucosphere:assistant` window event): Genie on the Coach page → then the MAS engine switch on Device-support, panel staying open through the transition — so the spotlight lands without a manual click. On those auto-driven steps the pause button reads **"Explore — I'll wait here"** (it's already open for you) rather than "Try it yourself."
- **Quick tour** rounded out to **7 steps** — detect → diagnose → assess → assistant → **Metrics Explained → About** — so even the fast path closes on "how it's computed" + the platform, then Done returns home.
- Spans `GuidedTour.jsx` + `tour/steps.js` (new `TOUR_STEPS_INTERACTIVE`) + anchors in `GlobalAssistant.jsx` / `MetricsExplained.jsx` / `AboutPage.jsx`.
...
### Added — `scripts/grant_viewers.py` (audience access: open the app + its About-page deep-links)
- Grants a **user / group / service principal** `CAN_USE` on the **deployed app itself** (so the audience can open it — apps are not in the generic `permissions update` object-type enum, so this goes through the dedicated `apps update-permissions`, additive PATCH that preserves the existing `CAN_MANAGE`/admins ACL), **plus** view-level access across every object the About-page "Under the hood" panel deep-links to — UC (`USE`/`SELECT`/`EXECUTE`/`READ VOLUME` via SQL GRANT), DLT pipeline (`CAN_VIEW`), job (`CAN_VIEW`), MLflow experiments (`CAN_READ`, scoped to the target's deploy path; notebook-backed experiments are granted on the backing notebook object, which the experiments permissions API rejects), forecast serving-endpoints (`CAN_QUERY`), KA + MAS Agent-Bricks tiles (`knowledge-assistants` / `supervisor-agents`, `CAN_QUERY`), and the Genie space (`CAN_RUN`). Permission levels + object types verified via `databricks permissions get-permission-levels` / `databricks apps get-permission-levels`. Principal type auto-detects (`@`→user, UUID→service-principal, else group) — handles the DAIS booth's user-fronting service principal. Dry-run by default. DEPLOY.md "Grant the audience" section documents it.
- **`--revoke`** reverses every grant for a principal across all surfaces (UC `REVOKE`; ACL objects via read-modify-write `set` of direct grants minus the principal, so inherited admin/manage is preserved) — for post-demo / teardown cleanup. No-op per object when the principal isn't on the ACL.
- **UC grant now checks the SQL `status.state`**, not just the HTTP code — the SQL-statements API returns HTTP 200 even on failure, so a failed `GRANT` previously printed a false `✓`. Failures now print `✗ [FAILED]`. In particular the built-in **workspace-local `users` group cannot hold UC grants** (`PRINCIPAL_DOES_NOT_EXIST`) — UC needs an account-level principal — so a whole-workspace `--principal users` grant covers the app + non-UC deep-links but not the Catalog/data deep-link (the script now says so explicitly).
