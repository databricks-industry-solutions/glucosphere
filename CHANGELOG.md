...
### Added — "Interactive" guided-tour variant (pause → try it → resume)
- A **third tour option** alongside Quick + Full (both unchanged, kept as backup). On steps that point at live behaviour — the Device-Support drill-in **AI "Deeper Analysis"** and the **assistant** — the card offers **"Try it yourself — I'll wait here"**: the overlay steps aside so the page is fully clickable, a floating **Resume · Step N/M** pill appears, and clicking it returns to the *same* step (no restart from step 1). Previously the dim overlay swallowed clicks and closed the tour, so these interactions never got showcased. Adds explicit stops on the assistant's **⚡ Fast ⇄ 🤖 MAS engine switch** and the **Genie data tab** (new `data-tour` anchors). Self-contained in `GuidedTour.jsx` + `tour/steps.js` (new `TOUR_STEPS_INTERACTIVE`) + two anchors in `GlobalAssistant.jsx`.
