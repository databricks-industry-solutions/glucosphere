...
- A **third tour option** alongside Quick + Full (Full unchanged; both kept as backup). On steps that point at live behaviour — the Device-Support drill-in **AI "Deeper Analysis"** and the **assistant** — the card offers **"Try it yourself — I'll wait here"**: the overlay steps aside so the page is fully clickable, a floating **Resume · Step N/M** pill (bottom-center, clear of the assistant "Ask" FAB) appears, and clicking it returns to the *same* step (no restart from step 1). Previously the dim overlay swallowed clicks and closed the tour, so these interactions never got showcased. The interactive arc adds explicit stops on the assistant's **⚡ Fast ⇄ 🤖 MAS engine switch** and the **Genie data tab**, and closes on **Metrics Explained → About** before returning home (new `data-tour` anchors: `assistant-engine`, `assistant-genie-tab`, `metrics-explained`, `about-hood`).
