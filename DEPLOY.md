...
**Easiest way — one command (recommended for contributors).** Stand up your own isolated sandbox (its own data + Genie/KA/MAS + app, never touching the live demo) by filling **4 lines** in `.env.bundle`, then running one script:

| Fill in (`.env.bundle`) | Plain meaning | Example |
|---|---|---|
| `BUNDLE_VAR_catalog` | which catalog the data goes in | `mmt_aws_usw2_catalog` |
| `BUNDLE_VAR_schema` | the "folder" name for the data | `glucosphere` |
| `DATABRICKS_CONFIG_PROFILE` | which workspace login to use | `fevm-mmt-aws-usw2` |
| `BUNDLE_VAR_dev_initials` | **your initials** — keeps your copy separate from everyone else's | `mmt` |

```bash
./scripts/deploy_sandbox.sh          # real data (HUPA-UCM); or: ./scripts/deploy_sandbox.sh synth_e2e
# → prints a plan → type y → ~50 min → your own app + agents
```

The script **auto-names your agents from your initials** (`Glucosphere_*_<initials>`) so they never collide with anyone else's, hides DABs' multi-pass deploy + agent-ID wiring, and never touches the live `gsphere` production deploy. `./scripts/deploy_sandbox.sh --help` shows options (data mode, custom suffix).

> ⚠️ **Cost:** a sandbox creates **billed** Agent Bricks endpoints (KA + MAS). When finished, tear it down — delete the app and its `Glucosphere_*_<initials>` tiles/spaces — to stop the charge.

> **Naming is workspace-global, so make the suffix unique.** The Genie/KA/MAS tiles are *workspace-wide* and matched **by name** — two deploys using the same suffix would share (collide on) the same agents. The initials default keeps yours unique; if you pass a custom suffix, pick something descriptive (e.g. a catalog tag like `_mmtcat`), not a generic one.

---

**Under the hood** (what the script automates — read this for manual control or debugging):
...
# Point the app at the new agents + redeploy — auto-discovers them by name
# (no hex-copying); reads harness_suffix from the target, finds Glucosphere_*<suffix>:
...
> **Watch the tile-vs-endpoint distinction.** The `_v2` suffix names the **tile / space**. But the KA and MAS **serving-endpoint** names are auto-generated by Agent Bricks as `ka-<hex>-endpoint` / `mas-<hex>-endpoint` (e.g. the live ones in `app.yaml` are `ka-4cd8c014-endpoint` / `mas-bae6759d-endpoint`) — those endpoint names (not the tile names) are what the app needs. `render_app_yaml.py --discover-agents` resolves them for you (it reads the tile detail's `serving_endpoint_name`), so you normally don't touch these by hand:

```bash
uv run python scripts/render_app_yaml.py --target gsphere --discover-agents
# …or pass them explicitly (overrides discovery; the Genie value is its space id):
uv run python scripts/render_app_yaml.py --target gsphere \
  --ka-endpoint ka-9f3a1c7d-endpoint --mas-endpoint mas-2b8e4f60-endpoint --genie-space-id 01f0a1b2c3d4e5f6a7b8c9d0e1f23456
```
