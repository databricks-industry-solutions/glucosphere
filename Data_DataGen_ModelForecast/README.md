...
| `05_incident_inference_bidirectional.py` | **Active** incident-simulation notebook. Two-incident mirror: Day 2 positive cohort (+40 mg/dL bias, over-reads), Day 5 negative cohort (-40 mg/dL bias, under-reads) — mutually exclusive 30%-of-fleet cohorts. **Also** injects the always-on, per-firmware zero-mean device-measurement-noise σ ramp (`%run _firmware_spec`, versions `3.14→4.0→4.0.3→4.1`) that produces the green→amber→red device-error gradient — distinct from the acute ±40 mg/dL incidents. Writes `pseudo_incident_7d_labeled` + `fleet_forecast_incident`; generates the 4 plot PNGs consumed by the App's MetricsExplained page. |
