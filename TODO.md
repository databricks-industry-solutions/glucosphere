...
- [ ] **Commit C** — replace stub `Data_DataGen_ModelForecast/01_ingest_real_baseline.py` with real implementation. Split into 4 sub-commits:
  ...
  - [x] **C.3** — implemented `from_table` mode (commit `1db686e`). Widgets: `BASELINE_SOURCE=from_table`, `SOURCE_CATALOG`, `SOURCE_SCHEMA`, `SOURCE_TABLE`. **2026-05-26 follow-up #72**: added prioritized auto-detect when widgets blank (`glucosphere_dev` → `glucosphere_from_source_e2e` → `glucosphere_synth_e2e`). Explicit widgets still win for deterministic harness use.
